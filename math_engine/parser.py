...
def parse_problem(problem: str) -> ProblemSpec:
    ...
    if m and ("面积" in problem or "周长" in problem):
        return ProblemSpec(
            problem_type="几何问题",
            sub_type="面积与周长",
            known={"length": float(m.group(1)), "width": float(m.group(2))},
            question={"target": "area" if "面积" in problem else "perimeter"},
            raw_text=problem,
            confidence=0.95,
        )

    # Fraction expressions must be matched before the generic arithmetic
    # expression, otherwise "1/2 + 1/3" is misread as "1 ÷ 2".
    ...
    if m:
        ...
        return ProblemSpec(
            problem_type="计算问题",
            sub_type="分数运算",
            question={"a": str(a), "op": m.group(3), "b": str(b)},
            raw_text=problem,
            confidence=0.98,
        )
    ...
    if m:
        ...
        return ProblemSpec(
            problem_type="计算问题",
            sub_type="四则运算",
            question={"a": m.group(1), "op": op, "b": m.group(3)},
            raw_text=problem,
            confidence=0.98,
        )
